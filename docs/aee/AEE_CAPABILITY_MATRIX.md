...
| alarms list/history | `/api/v1/AlarmList`, query filters, non-empty rows and handling projection | LIVE+STATIC PARTIAL | Class A | read-only Adapter and raw-code `AlarmEvent` normalization implemented; not persisted | complete code maps, lifecycle, retention and privacy rules before persistence |
