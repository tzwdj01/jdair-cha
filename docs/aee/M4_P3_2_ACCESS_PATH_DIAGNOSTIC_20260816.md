...
从 CHA 生产服务器（egress public IP `<CHA_PRODUCTION_EGRESS_IP>`）请求同一批 API
...
* `aee.jdcloud.com` → A record `<AEE_EDGE_IP>`（单条 A；AAAA 无）。
...
* `LocalConfig.xml` `ServerModel.IpAddress = <MCS8_SERVER_HOST>`（MCS8 SDK 服务器，
  与 `aee.jdcloud.com` 前端 IP `<AEE_EDGE_IP>` 不同）。
* 登录：`mcs8_ws_login` → WebSocket 到 `<MCS8_SERVER_HOST>:7711`
  (`/ ?uid=<user>&pwd=<md5>`)，返回 `token`（160 字符）。
* 数据：`call_mcs8_api` → HTTP 到 `<MCS8_SERVER_HOST>:7712`
  （`api_port = FilePort+5`），带 `token` header + `SessionId` 参数。

在 CHA 服务器上实测该通道（使用 LocalConfig 账号，通过 WS 登录获取新 token，
只读请求）：

| API（<MCS8_SERVER_HOST>:7712） | HTTP | 结果 |
| --- | --- | --- |
...
即：**`<MCS8_SERVER_HOST>:7712`（MCS8 原生服务器）已被 CHA Legacy 生产使用且可用**，
...
（`<CHA_PRODUCTION_EGRESS_IP>`）未放行，`/api/v1/*` 与 `/v3/*` URI 命中 deny 规则 →
...
1. **MCS8 原生服务器通道（首选，已存在且受支持）**：
   `<MCS8_SERVER_HOST>:7711`（WS 登录）→ `:7712`（REST，token+SessionId）。
   Legacy 已在生产使用；M4 adapter 复用即可，无需 WAF 变更。
   DevOnlineList 以 `GetDevListByGroupId`（含 nOnline）或等效在线状态源替代。
2. JD Cloud WAF/JFE 对 CHA 固定出口 IP `<CHA_PRODUCTION_EGRESS_IP>` 合法白名单
