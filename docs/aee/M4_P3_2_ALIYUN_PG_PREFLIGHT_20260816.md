...
(`47.251.105.9`, root). Read-only audit only; **no software installed**.
...
| Public / private IP | 47.251.105.9 / 172.17.17.167 |
...
CHA production host `jdair.top` → `111.228.15.31` (China Telecom range;
...
* CHA: tailscaled present, Tailscale IP `100.74.86.85` (prior audit).
