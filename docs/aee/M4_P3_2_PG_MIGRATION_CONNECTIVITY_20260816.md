...
Security re-confirmed: listen `127.0.0.1:5432` + `<PG_TAILSCALE_IP>:5432`;
public `<PG_CANDIDATE_PUBLIC_IP>:5432` CLOSED; pg_hba minimal (app/migrator only from
`<CHA_TAILSCALE_IP>/32` scram, localhost scram, reject `0.0.0.0/0`+`::/0`).
...
From CHA host (`cn-edge` `<CHA_TAILSCALE_IP>`) via Tailscale to Aliyun
`<PG_TAILSCALE_IP>:5432`: TCP OK; authenticated `psql` returned
...
`CHA_PG_HOST=<PG_TAILSCALE_IP>` (Tailscale IP, not public), `PORT/DATABASE/
