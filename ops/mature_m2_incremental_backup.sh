...
backup_name="${stamp}-before-m2-dashboard"
backup_base="${backup_root}/${backup_name}"
backup_archive="${backup_root}/jdair-cha-before-m2-dashboard-${stamp}.tar.gz"
...
test "$v2_target" = "/opt/jdair-cha/v2/releases/20260813215638-m1-legacy-adapter"
test "$(systemctl is-active jdair-cha.service)" = "active"
test "$(systemctl is-active jdair-cha-v2.service)" = "active"
test "$(tr -d '\r\n[:space:]' < "$v2_target/VERSION")" = "0.2.0"
test "$(tr -d '\r\n[:space:]' < "$v2_target/BUILD")" = "m1-legacy-adapter"
nginx -t
...

{
  ...
  printf 'purpose=before-m2-dashboard\n'
...
test "\$(curl -sS -o /tmp/jdair-cha-m2-rollback-health.json -w '%{http_code}' --max-time 15 http://127.0.0.1:8791/api/v2/health)" = "200"
grep -q '"version":"0.2.0"' <(curl -sS --max-time 15 http://127.0.0.1:8791/api/v2/system/version)
grep -q '"dashboard_v2":false' <(curl -sS --max-time 15 http://127.0.0.1:8791/api/v2/system/features)
