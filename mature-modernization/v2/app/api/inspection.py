...
from .inspection_access import InspectionAccess
...
def create_inspection_router(
    ...
    access: InspectionAccess | None = None,
) -> APIRouter:
    ...
    * store wired: computed metrics from durable rows, or empty result sets
    ...
    async def require_access(request: Request) -> JSONResponse | None:
        """Fail closed when Phase 6 data access is not explicitly wired."""

        if access is None:
            return envelope(
                request,
                {
                    "code": "inspection_access_not_configured",
                    "message": "Inspection access enforcement is not configured.",
                },
                ok=False,
                status_code=503,
            )
        _identity, error = await access.require_authorized(request)
        return error

    async def data_gate(request: Request) -> JSONResponse | None:
        """Apply the feature and AuthorizedUser gates in a stable order."""

        if not settings.feature_inspection_v2:
            return disabled(request)
        return await require_access(request)
    ...
    for page_name in PAGE_ROUTES:
        ...
        async def inspection_page(
            ...
            return await render_page(request, _page_name)
    ...
    async def inspection_overview_page(request: Request) -> Response:
        return await render_page(request, "overview")
    ...
    for _alias, _target in (
        ...
        async def inspection_alias_page(
            ...
            return await render_page(request, _target_name)
