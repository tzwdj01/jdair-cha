...
def create_inspections_router(
    ...
    async def inspections_page(request: Request) -> Response:
        if not settings.feature_inspection_v2:
            return HTMLResponse(
                status_code=404,
                content="<meta charset='utf-8'><h1>监察记录页未启用</h1>",
            )
        _identity, access_error = await access.require_authorized(request)
    ...
    async def list_inspections(
        ...
        if blocked is not None:
            return blocked
        _identity, _auth_error = await access.require_authorized(request)
    ...
    async def list_authorized_users(request: Request) -> JSONResponse:
        ...
        if blocked is not None:
            return blocked
        identity, error = await access.require_admin(request)
