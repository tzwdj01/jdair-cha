...
class _FakePool:
    ...
    def __init__(self, minimum: int, maximum: int, **kwargs) -> None:
        self.minimum = minimum
        self.maximum = maximum
        self.kwargs = kwargs
        self.available: list[_FakeConnection] = []
        self.get_calls = 0
        self.returns: list[tuple[_FakeConnection, bool]] = []
        self.closed = False
    ...
    def putconn(self, conn: _FakeConnection, close: bool = False) -> None:
...
class PooledConnectionTests(unittest.TestCase):
    ...
    def _pool(self) -> PostgresConnectionPool:
    ...
    def test_context_manager_commits_returns_and_reuses_connection(self) -> None:
        with patch(
            ...
            with pool.connection() as first:
                self.assertIsInstance(first, _FakeConnection)
    ...
    def test_context_manager_rolls_back_and_returns_healthy_connection(self) -> None:
        with patch(
            ...
            fake_pool = _FakePool.instances[0]
            returned, close = fake_pool.returns[0]
            self.assertEqual(returned.rollbacks, 1)
            self.assertFalse(close)

    def test_failed_commit_discards_broken_connection(self) -> None:
        with patch(
            "app.data.store.pool._ThreadedConnectionPool",
            _FakePool,
        ):
            pool = self._pool()
            fake_pool = pool._get_pool()
            broken = _FakeConnection(fail_commit=True)
            fake_pool.available.append(broken)
            with self.assertRaisesRegex(RuntimeError, "commit failed"):
                with pool.connection():
                    pass
            self.assertEqual(fake_pool.returns, [(broken, True)])

    def test_failed_rollback_discards_broken_connection(self) -> None:
        with patch(
            "app.data.store.pool._ThreadedConnectionPool",
            _FakePool,
        ):
            pool = self._pool()
            fake_pool = pool._get_pool()
            broken = _FakeConnection(fail_rollback=True)
            fake_pool.available.append(broken)
    ...
    def test_data_and_workflow_store_pool_bounds_remain_small(self) -> None:
        data_store = PostgresInspectionStore(
            host="127.0.0.1",
            database="cha_m4_test",
            user="cha_test",
            sslmode="disable",
        )
        workflow_store = PostgresInspectionRecordStore(
            host="127.0.0.1",
            database="cha_m4_test",
            user="cha_test",
            sslmode="disable",
        )
        self.assertEqual(data_store._pool._max_connections, 4)
        self.assertEqual(workflow_store._pool._max_connections, 2)
        self.assertFalse(data_store._pool.initialized)
    ...
    def test_store_health_checks_use_bounded_connection_contexts(self) -> None:
        ...
        with patch.object(data_store, "_connect", fake_connection):
            self.assertTrue(asyncio.run(data_store.health_check()))
        with patch.object(workflow_store, "_connect", fake_connection):
            self.assertTrue(asyncio.run(workflow_store.health_check()))
