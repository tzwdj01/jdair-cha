...
def _app(username: str | None = "inspector-a", candidate_service=None):
    ...
    app.include_router(
        create_inspections_router(
            ...
            store,
            _envelope,
            InspectionAccess(store, _Identity(username), _envelope),
            candidate_service,
        )
    )
    return app, store
...
class InspectionsAPITests(unittest.IsolatedAsyncioTestCase):
    async def test_workflow_page_and_apis_share_authorized_user_boundary(
        ...
        anonymous_app, anonymous_store = _app(username=None)
        ...
        for path in (
            ...
            response = await _request(anonymous_app, "GET", path)
            self.assertEqual(response.status_code, 401, path)

        disabled_app, disabled_store = _app(username="disabled-a")
        await _seed_authorized(disabled_store)
        await disabled_store.upsert_authorized_user(
            build_authorized_user(
                aee_account_id="acc-disabled",
                username="disabled-a",
                enabled=False,
            )
        )
        response = await _request(disabled_app, "GET", "/api/v2/inspections")
        self.assertEqual(response.status_code, 403)

        inspector_app, inspector_store = _app(username="inspector-a")
        await _seed_authorized(inspector_store)
        self.assertEqual(
            (
                await _request(
                    inspector_app,
        ...
        admin_app, admin_store = _app(username="admin-a")
        await _seed_authorized(admin_store)
        self.assertEqual(
            (
                await _request(
                    admin_app,
    ...
    async def test_csv_export_excludes_secrets(self) -> None:
        ...
        response = await _request(
            ...
            "GET",
            (
                "/api/v2/inspections/export?fmt=csv"
