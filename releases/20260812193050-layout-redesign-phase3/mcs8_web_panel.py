...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Phase 2/3: theme switching, modal player, compact rows and module tabs. */
  ...
  <main>
    ...
    <section id="view-aux" class="view">
      ...
      <div class="panel" id="routineDetailPanel" style="display:none;">
        ...
        <aside class="aux-info-panel" aria-label="系统功能和版本记录">
  ...
  try {
    ...
    byId("records").innerHTML = list.length ? list.map((item, index) => {
      ...
      return `<tr id="record-row-${referenceGeneration}-${index}">
        <td>${esc(dev)}</td><td>${esc(item.deviceName || item.devName || "")}</td><td class="file-title-cell"><div>${esc(title)}</div></td>
