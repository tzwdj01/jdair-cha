...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Phase 2: theme switching, modal video player, compact rows and module tabs. */
    ...
    .records-table th:nth-child(1), .records-table td:nth-child(1) { width: 86px; }
    .records-table th:nth-child(2), .records-table td:nth-child(2) { width: 104px; }
    .records-table th:nth-child(3), .records-table td:nth-child(3) { width: 250px; }
    .records-table th:nth-child(4), .records-table td:nth-child(4) { width: 108px; }
    .records-table th:nth-child(5), .records-table td:nth-child(5) { width: 92px; }
    .records-table th:nth-child(6), .records-table td:nth-child(6),
    .records-table th:nth-child(7), .records-table td:nth-child(7) { width: 112px; }
    .records-table th:nth-child(8), .records-table td:nth-child(8) { width: 270px; }
    .records-table th:nth-child(9), .records-table td:nth-child(9) { width: 186px; }
    .records-table tbody tr:not(.reference-expanded) { height: 68px; }
  ...
  <main>
    ...
    <section id="view-aux" class="view">
      <div class="view-heading">
        ...
        <p>在航班动态和例行任务之间切换，当前页面只显示所选查询模块</p>
      </div>
      <div class="module-workspace">
...
function showAuxPane(name) {
  activeAuxPane = name === "routine" ? "routine" : "flights";
  ["flights", "routine"].forEach(function(pane) {
    ...
    if (el) el.classList.toggle("active", pane === activeAuxPane);
    if (btn) btn.classList.toggle("active", pane === activeAuxPane);
  });
  if (activeAuxPane === "flights" && !flightsLoaded) resetFlightSearch();
  if (activeAuxPane === "routine" && !routineLoaded) resetRoutineSearch();
}
...
async function loadRecordFlightReferences(list, generation) {
  ...
  try {
    ...
    (payload.data || []).forEach(function(ref, index) {
      ...
      if (cell) {
        cell.innerHTML = `<div class="reference-content">${renderFlightReference(ref)}</div><button class="reference-toggle" onclick="toggleReferenceInfo('${cell.id}', this)">展开</button>`;
      }
    });
  } catch (err) {
    if (generation !== recordReferenceGeneration) return;
    list.forEach(function(_, index) {
      const cell = document.getElementById(`flight-ref-${generation}-${index}`);
      if (cell) cell.innerHTML = `<div class="reference-content"><div class="flight-ref-empty">参考信息载入失败：${esc(err.message || err)}</div></div><button class="reference-toggle" onclick="toggleReferenceInfo('${cell.id}', this)">展开</button>`;
    });
  }
}

function toggleReferenceInfo(cellId, button) {
  const cell = byId(cellId);
  if (!cell) return;
  const expanded = !cell.classList.contains("reference-expanded");
  ...
  try {
    ...
    byId("records").innerHTML = list.length ? list.map((item, index) => {
      ...
      return `<tr id="record-row-${referenceGeneration}-${index}">
        <td>${esc(dev)}</td><td>${esc(item.deviceName || item.devName || "")}</td><td class="file-title-cell"><div>${esc(title)}</div></td><td>${esc(item.fileType || item.type || item.fType || "视频/普通文件")}${fileMode === "device" ? " / 平台回退" : ""}</td>
        ...
        <td class="flight-ref" id="flight-ref-${referenceGeneration}-${index}"><div class="reference-content"><span class="flight-ref-empty">正在匹配参考信息…</span></div><button class="reference-toggle" disabled>展开</button></td>
...
document.addEventListener("keydown", function(event) {
  if (event.key === "Escape" && byId("playerModal")?.classList.contains("open")) {
    closePlayerModal();
  }
});
applyPreset();
